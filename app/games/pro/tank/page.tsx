...
function TankArenaPage() {
  ...
  const [status, setStatus] = useState<
    "checking" | "ready" | "playing" | "submitting" | "finished" | "error"
  >("checking");
  ...
  useEffect(() => {
    ...
    async function checkAccess() {
      try {
        ...
        if (tournament.game_type !== "tank_arena") {
          setError("Ce challenge n'est pas un tournoi Tank Arena.");
          setStatus("error");
          return;
        }
        ...
        if (entry.score_submitted_at) {
          setMyScore(Number(entry.score) || 0);
          setStatus("finished");
          return;
        }
        ...
        const gameHTML = getGameHTML(false);
  ...
  useEffect(() => {
    const onMessage = (e: MessageEvent) => {
      ...
      if (e.data?.type === "TANK_GAME_OVER") {
        handleFinish(Number(e.data.score) || 0);
      }
    };
    ...
    return (
      <main className="min-h-screen bg-[#05070c] text-white flex items-center justify-center px-5">
        <div className="text-center">
          ...
          <p className="font-black text-zinc-300">Vérification du Tank Challenge...</p>
        </div>
      </main>
    );
  }
  ...
  if (status === "finished") {
    return (
      <main className="min-h-screen bg-[#05070c] text-white flex items-center justify-center px-5">
        <div className="w-full max-w-xl rounded-3xl border border-green-400/30 bg-zinc-950 p-8 text-center">
          ...
          <p className="mt-3 text-zinc-400">
            Belle bataille ! Ton score est maintenant dans le classement du tournoi.
          </p>
  ...
  return (
    <div className="fixed inset-0 bg-[#05070c]">
      {status === "ready" && (
        <div
          className="absolute top-4 left-1/2 -translate-x-1/2 z-10 px-4 py-2 rounded-full text-xs font-bold"
          style={{
        ...
        >
          🏆 Tank Arena Pro — Une seule tentative
        </div>
      )}
...
export default function TankArenaPageWrapper() {
  return <TankArenaPage />;
}


// ── HTML du jeu (ton code original intact) ──
...
// ── En mode duel, on n'envoie le score qu'une seule fois
...
renderLeaders();
function saveLeader(){leaders.push({score,wave,date:Date.now()});leaders.sort((a,b)=>b.score-a.score);leaders=leaders.slice(0,5);}
