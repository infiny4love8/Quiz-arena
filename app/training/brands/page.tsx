...
import brandsQuestions from "@/data/brands.json";

const QUESTION_LIMIT = 10;
const TIME_PER_QUESTION = 10;
const SCORE_STORAGE_KEY = "brands-training-scores";
...
function shuffleOptionsBalanced(options: string[]) {
  return fisherYates(options);
}

function prepareQuestions(): Question[] {
  return fisherYates(brandsQuestions)
    .slice(0, QUESTION_LIMIT)
    .map((q: Question) => ({
      ...q,
      options: shuffleOptionsBalanced(q.options),
    }));
}
...
export default function BrandsTrainingPage() {
      ...
      <main className="min-h-screen bg-black text-white overflow-hidden flex items-center justify-center px-4">
        ...
        <div className="relative z-10 w-full max-w-2xl rounded-[2rem] border border-white/10 bg-white/5 p-8 md:p-12 text-center shadow-[0_0_120px_rgba(0,0,0,0.7)] backdrop-blur-xl">
          ...
          <p className="mt-4 text-white/60 max-w-md mx-auto">
            Reconnais les marques, gagne des points, enchaîne les combos et garde tes scores.
          </p>
          <button
            ...
            <div className="flex justify-center">
              <div className="rounded-3xl border border-white/10 bg-black/40 p-3 shadow-2xl">
                <Image
                  ...
                  width={320}
                  height={180}
                  className="h-40 w-auto rounded-2xl object-contain md:h-48"
                  unoptimized
                />
              </div>
            </div>
                  ...
                  >
                    <span className="flex items-center justify-between gap-4">
                      ...
                      <span className="text-xs uppercase tracking-[0.25em] opacity-70">
                        {showResult && isCorrect ? "Juste" : showResult && isSelected ? "Faux" : ""}
                      </span>
                    </span>
                  </button>
                );
