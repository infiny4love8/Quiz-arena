...
function TankArenaPage() {
  ...
  // ── Guard : on n'envoie le score qu'une seule fois
  ...
  const handleFinish = useCallback(async (score: number) => {
    // ── Guard : ne traiter qu'une seule fois même si le joueur rejoue
    ...
    if (oppScore !== null) {
      ...
      // ── Transfert des coins si pari
      if (
        updated.bet_a && updated.bet_b &&
    ...
    <div className="fixed inset-0 bg-[#05070c]">
      {isDuelMode && !gameStarted && (
        <div
          ...
          style={{
            background: "rgba(80,255,208,0.15)",
            border: "1px solid rgba(80,255,208,0.4)",
            color: "#50ffd0",
          }}
        >
