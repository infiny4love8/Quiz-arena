...
const GAME_META: Record<string, { icon: string; label: string; accent: string; bg: string; glow: string }> = {
  flags:  { icon: "🌍", label: "Drapeaux du monde", accent: "#22c55e", bg: "#0a1f13",  glow: "#22c55e" },
  memory: { icon: "🧠", label: "Memory Rush",        accent: "#ef4444", bg: "#1a0a0a",  glow: "#ef4444" },
  tank:   { icon: "🎯", label: "Tank Arena",          accent: "#06b6d4", bg: "#050f18",  glow: "#06b6d4" },
};
...
function playClickSound() {
  ...
  osc.type = "sine"; osc.frequency.value = 440; gain.gain.value = 0.03;
    ...
    <div style={{
      ...
      <div style={{
        ...
        <p style={{
          ...
          Si tu quittes maintenant, <span style={{ color: "#f59e0b", fontWeight: 700 }}>ton score actuel sera soumis automatiquement</span> et tu ne pourras plus rejouer ce tournoi.
        </p>
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          <button onClick={onStay} style={{
            background: `linear-gradient(135deg, ${accent}, ${accent}cc)`,
            border: "none", borderRadius: 12,
            color: "#0a0c10", padding: "14px",
          ...
          <button onClick={onQuit} style={{
            ...
            Soumettre mon score et quitter
          </button>
        </div>
      </div>
    </div>
...
function GameFlags({
  ...
  // Notifie le parent du score partiel en temps réel
  useEffect(() => { onPartialScore(points); }, [points, onPartialScore]);

  useEffect(() => {
    if (isFinished && !scoreSent.current) { scoreSent.current = true; onScore(points); }
  }, [isFinished, points, onScore]);

  useEffect(() => {
    if (tournamentSecsLeft <= 0 && !scoreSent.current) { scoreSent.current = true; onScore(points); }
  ...
  if (!currentQuestion) return (
    <div style={{ textAlign: "center", padding: "40px 0", color: "#4a5568" }}>Chargement…</div>
  );
  ...
  return (
    <div style={{ animation: "fadeUp 0.3s ease" }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <span style={{
            ...
            {currentIndex + 1}<span style={{ color: "#1e2130" }}>/</span>{questions.length}
          </span>
          {combo > 1 && (
            <span style={{
      ...
      {/* Barre timer */}
      <div style={{ height: 5, background: "#1e2130", borderRadius: 99, marginBottom: 4, overflow: "hidden" }}>
        <div style={{
      ...
      <div style={{ height: 3, background: "#1e2130", borderRadius: 99, marginBottom: 22 }}>
        <div style={{
          height: "100%", borderRadius: 99,
      ...
      <div style={{ textAlign: "center", marginBottom: 22 }}>
        ...
        <h2 style={{
          ...
          color: "#f0ede6", marginTop: 16, marginBottom: 0,
          lineHeight: 1.3,
        }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
        {currentQuestion.options.map((o) => {
          ...
          let bg = "#0d0f14", border = "#1e2130", color = "#c8c0b0";
          let shadow = "none";
          if (showResult && isCorrect) {
            bg = "#0d2e1f"; border = "#22c55e"; color = "#22c55e";
            shadow = "0 0 16px #22c55e30";
          } else if (showResult && isSelected && !isCorrect) {
            bg = "#2d0f0f"; border = "#ef4444"; color = "#ef4444";
          }
          return (
            <button
              key={o}
      ...
      <div style={{ textAlign: "center", marginTop: 18, fontSize: 13, color: "#4a5568" }}>
        Score : <span style={{ color: "#22c55e", fontWeight: 800 }}>{points} pts</span>
      </div>
    </div>
  );
}
...
// ─────────────────────────────────────────────
// JEU MEMORY (mode Normal imposé)
// ─────────────────────────────────────────────
type EmojiItem = { emoji: string; category: "fruit"|"transport"|"animal"|"symbol"; };
type QuestionType = "position-of-emoji"|"emoji-at-position"|"count-category"|"missing-emoji";
type MemRound = { sequence: EmojiItem[]; questionType: QuestionType; question: string; options: string[]; answer: string; };

const EMOJIS: EmojiItem[] = [
  { emoji:"🍎",category:"fruit"},{emoji:"🍓",category:"fruit"},{emoji:"🍒",category:"fruit"},
  { emoji:"🍉",category:"fruit"},{emoji:"🍊",category:"fruit"},{emoji:"🍌",category:"fruit"},
  { emoji:"🍇",category:"fruit"},{emoji:"🥝",category:"fruit"},{emoji:"🍍",category:"fruit"},
  { emoji:"🚗",category:"transport"},{emoji:"🚌",category:"transport"},{emoji:"✈️",category:"transport"},
  { emoji:"🚲",category:"transport"},{emoji:"🚢",category:"transport"},{emoji:"🚁",category:"transport"},
  { emoji:"🐶",category:"animal"},{emoji:"🐱",category:"animal"},{emoji:"🦁",category:"animal"},
  { emoji:"🐸",category:"animal"},{emoji:"🐼",category:"animal"},{emoji:"🦊",category:"animal"},
  { emoji:"⭐",category:"symbol"},{emoji:"❤️",category:"symbol"},{emoji:"🔥",category:"symbol"},
  { emoji:"⚡",category:"symbol"},{emoji:"💎",category:"symbol"},{emoji:"🏆",category:"symbol"},
];

const MEM_ROUNDS      = 10;
const MEM_MEMORIZE_MS = 2500;
const MEM_ANSWER_SECS = 6;
const MEM_SEQ_SIZE    = 5;

function getCatLabel(c: EmojiItem["category"]) {
  return { fruit:"fruits", transport:"transports", animal:"animaux", symbol:"symboles" }[c];
}

function buildMemRound(idx: number): MemRound {
  const seq = fisherYates(EMOJIS).slice(0, MEM_SEQ_SIZE);
  const types: QuestionType[] = ["position-of-emoji","emoji-at-position","count-category","missing-emoji"];
  const qt = types[idx % types.length];
  if (qt === "position-of-emoji") {
    const ti = Math.floor(Math.random() * seq.length);
    return { sequence: seq, questionType: qt, question: `Où était placé ${seq[ti].emoji} ?`, options: Array.from({length:seq.length},(_,i)=>String(i+1)), answer: String(ti+1) };
  }
  if (qt === "emoji-at-position") {
    const ti = Math.floor(Math.random() * seq.length);
    const correct = seq[ti].emoji;
    const wrong = fisherYates(EMOJIS.filter(i=>i.emoji!==correct).map(i=>i.emoji)).slice(0,3);
    return { sequence: seq, questionType: qt, question: `Quel emoji était en position ${ti+1} ?`, options: fisherYates([correct,...wrong]), answer: correct };
  }
  if (qt === "count-category") {
    const cats: EmojiItem["category"][] = ["fruit","transport","animal","symbol"];
    const cat = cats[Math.floor(Math.random()*cats.length)];
    const count = seq.filter(i=>i.category===cat).length;
    const opts = fisherYates([...new Set(["0","1","2","3","4","5"].filter(v=>v!==String(count)))]).slice(0,3);
    return { sequence: seq, questionType: qt, question: `Combien de ${getCatLabel(cat)} étaient affichés ?`, options: fisherYates([String(count),...opts]), answer: String(count) };
  }
  const mi = Math.floor(Math.random()*seq.length);
  const missing = seq[mi].emoji;
  const wrong = fisherYates(EMOJIS.filter(i=>i.emoji!==missing).map(i=>i.emoji)).slice(0,3);
  return { sequence: seq, questionType: qt, question: "Quel emoji faisait partie de la séquence ?", options: fisherYates([missing,...wrong]), answer: missing };
}

type MemPhase = "memorize"|"question"|"feedback";
...
function GameMemory({
  ...
  const [round, setRound]               = useState<MemRound>(() => buildMemRound(0));
  const [phase, setPhase]               = useState<MemPhase>("memorize");
  const [memorizeProgress, setMemoProgress] = useState(100);
  const [answerTime, setAnswerTime]     = useState(MEM_ANSWER_SECS);
  const [selected, setSelected]         = useState("");
  const [score, setScore]               = useState(0);
  const [feedback, setFeedback]         = useState<"correct"|"wrong"|null>(null);
  const scoreRef                        = useRef(0);
  const memorizeTimer                   = useRef<NodeJS.Timeout|null>(null);

  // Notifie le parent
  useEffect(() => { onPartialScore(scoreRef.current); });

  function startMemorize(idx: number) {
    const r = buildMemRound(idx);
    setRound(r); setPhase("memorize"); setSelected(""); setFeedback(null);
    setMemoProgress(100);
    const start = Date.now();
    if (memorizeTimer.current) clearInterval(memorizeTimer.current);
    memorizeTimer.current = setInterval(() => {
      const elapsed = Date.now() - start;
      const pct = Math.max(0, 100 - (elapsed / MEM_MEMORIZE_MS) * 100);
      setMemoProgress(pct);
      if (elapsed >= MEM_MEMORIZE_MS) {
        clearInterval(memorizeTimer.current!);
        setAnswerTime(MEM_ANSWER_SECS);
        setPhase("question");
      }
    }, 50);
  }

  useEffect(() => { startMemorize(0); return () => { if (memorizeTimer.current) clearInterval(memorizeTimer.current); }; }, []);

  useEffect(() => {
    if (phase !== "question") return;
    if (answerTime <= 0) { submitAnswer(""); return; }
  ...
  function submitAnswer(ans: string) {
    if (phase !== "question") return;
    const correct = ans === round.answer;
    const gained  = correct ? (answerTime >= 4 ? 12 : 10) : 0;
    const next = scoreRef.current + gained;
    scoreRef.current = next;
    setScore(next); setSelected(ans);
    setFeedback(correct ? "correct" : "wrong");
    setPhase("feedback");
    setTimeout(() => {
      if (roundIdx + 1 < MEM_ROUNDS) {
        const nextIdx = roundIdx + 1;
        setRoundIdx(nextIdx);
        startMemorize(nextIdx);
      } else {
        onScore(next);
      }
    }, 950);
  ...
  const accent = "#ef4444";
  const timerPct = (answerTime / MEM_ANSWER_SECS) * 100;

  return (
    <div style={{ animation: "fadeUp 0.3s ease" }}>
      {/* Stats */}
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
        <span style={{ fontFamily: "'Share Tech Mono', monospace", fontSize: 13, color: "#6b7280" }}>
          Round {roundIdx+1}/{MEM_ROUNDS}
        </span>
        <span style={{ fontFamily: "'Share Tech Mono', monospace", fontSize: 14, fontWeight: 900, color: accent }}>
          {score} pts
        </span>
        {phase === "question" && (
          <span style={{ fontSize: 13, fontWeight: 700, color: answerTime <= 2 ? "#ef4444" : "#f59e0b" }}>
            {answerTime}s
          </span>
        )}
      </div>
      ...
      {/* Timer bar */}
      {phase === "question" && (
        <div style={{ height: 4, background: "#1e2130", borderRadius: 99, marginBottom: 18, overflow: "hidden" }}>
          <div style={{
            height: "100%", borderRadius: 99,
      ...
      {phase === "memorize" && (
        <div style={{ textAlign: "center" }}>
          ...
          <div style={{ height: 5, background: "#1e2130", borderRadius: 99, marginBottom: 24, overflow: "hidden" }}>
            <div style={{
              height: "100%", borderRadius: 99,
              background: `linear-gradient(90deg, ${accent}80, ${accent})`,
              width: `${memorizeProgress}%`, transition: "width 0.05s linear",
            }} />
          </div>
          <div style={{ display: "flex", flexWrap: "wrap", justifyContent: "center", gap: 12 }}>
            {round.sequence.map((item, i) => (
              <div key={i} style={{
                ...
                fontSize: 42, boxShadow: "0 4px 20px rgba(0,0,0,0.4)",
                animation: `scaleIn 0.3s cubic-bezier(0.34,1.56,0.64,1) ${i*0.06}s both`,
              }}>
                {item.emoji}
              </div>
            ))}
          ...
          <p style={{ fontSize: 12, color: "#4a5568", marginTop: 20 }}>
            La question arrive dans un instant…
          </p>
        </div>
      )}
      ...
      {(phase === "question" || phase === "feedback") && (
        <div>
          <div style={{
            ...
            <h2 style={{ fontSize: 16, fontWeight: 800, color: "#f0ede6", margin: 0 }}>
              {round.question}
            </h2>
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
            {round.options.map((o) => {
              ...
              const isCorrect  = o === round.answer;
              const showResult = phase === "feedback";
              let bg = "#0d0f14", border = "#1e2130", color = "#c8c0b0";
              if (showResult && isCorrect)                  { bg="#0d2e1f"; border="#22c55e"; color="#22c55e"; }
              else if (showResult && isSelected && !isCorrect) { bg="#2d0f0f"; border=accent; color=accent; }
              return (
                <button key={o} onClick={() => submitAnswer(o)} disabled={phase !== "question"}
                  style={{
                    background: bg, border: `2px solid ${border}`,
          ...
          {phase === "feedback" && feedback && (
            <div style={{
              marginTop: 16, padding: "12px 14px", borderRadius: 12, textAlign: "center",
              background: feedback === "correct" ? "#0d2e1f" : "#2d0f0f",
              border: `1px solid ${feedback === "correct" ? "#22c55e40" : "#ef444440"}`,
              color: feedback === "correct" ? "#22c55e" : "#ef4444",
              fontSize: 14, fontWeight: 700,
              animation: "scaleIn 0.2s ease",
            }}>
              {feedback === "correct" ? "✓ Correct !" : `✗ Réponse : ${round.answer}`}
            </div>
          )}
        </div>
      )}
...
function GameTank({
  ...
  useEffect(() => {
    const onMessage = (e: MessageEvent) => {
      if (e.data?.type === "TANK_SCORE_UPDATE") {
        currentScore.current = e.data.score ?? 0;
        onPartialScore(currentScore.current);
      }
      if (e.data?.type === "TANK_STARTED") setGameStarted(true);
      if (e.data?.type === "TANK_GAME_OVER") {
        if (scoreSent.current) return;
        scoreSent.current = true;
        onScore(e.data.score ?? 0);
      }
    };
    window.addEventListener("message", onMessage);
    return () => window.removeEventListener("message", onMessage);
  }, [onScore, onPartialScore]);

  // Exposer getScore pour l'anti-triche
  useEffect(() => {
    (window as any).__tankGetScore = () => currentScore.current;
...
function PlayContent() {
  ...
  useEffect(() => {
    const init = async () => {
      ...
      const { data: entry } = await supabase
        .from("tournament_entries").select("finished")
        .eq("tournament_id", tournamentId).eq("user_id", user.id).single();
      if (!entry || entry.finished) { router.push("/tournamentsponsorise"); return; }

      const { data: t } = await supabase
        .from("tournaments").select("id, title, game_type, ends_at, prize_1, prize_2")
  ...
  // ── Anti-triche : intercepte navigation pendant la partie
  useEffect(() => {
    if (!isPlaying) return;
    ...
    // Pousse un état pour catcher le retour arrière
    window.history.pushState(null, "", window.location.href);
  ...
  const handleScore = useCallback(async (score: number) => {
    ...
    if (error || !data?.ok) { setErrorMsg("Erreur lors de la soumission du score."); setPageState("error"); return; }

    const { data: scores } = await supabase
      .from("tournament_entries").select("user_id, score")
      .eq("tournament_id", tournamentId).eq("finished", true)
  ...
  // ── Score partiel (anti-triche)
  const handlePartialScore = useCallback((score: number) => {
    partialRef.current = score;
    setPartialScore(score);
  }, []);

  // ── Quitter depuis la modale
  const handleQuitAndSubmit = useCallback(() => {
    setShowModal(false);
    // Récupère le score tank si applicable
    const score = gameType === "tank"
      ? ((window as any).__tankGetScore?.() ?? partialRef.current)
      : partialRef.current;
    handleScore(score);
  }, [gameType, handleScore]);

  // ─── RENDER ───
  if (pageState === "loading") return (
    <div style={pageCentered}>
      <div style={{ ...spinnerStyle, borderTopColor: meta.accent }} />
  ...
  if (pageState === "error") return (
    <div style={pageCentered}>
      ...
      <button onClick={() => router.push("/tournamentsponsorise")} style={btnSecondary}>Retour</button>
    </div>
  );
  ...
  if (pageState === "ready") return (
    <>
      <style>{css}</style>
      <div style={{ ...pageWrap, background: `radial-gradient(ellipse at 50% 0%, ${meta.bg} 0%, #0a0c10 60%)` }}>
        <div style={card}>
          ...
          <div style={{ textAlign: "center", marginBottom: 28 }}>
            <div style={{
              ...
              boxShadow: `0 0 40px ${meta.accent}20`,
            }}>
              {meta.icon}
            </div>
            <div style={{ fontSize: 10, color: "#4a5568", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.14em", marginBottom: 6 }}>
              Tournoi Sponsorisé · 4Infiny
            </div>
            <h1 style={{ fontFamily: "'Bebas Neue', cursive", fontSize: 28, color: "#f0ede6", letterSpacing: "0.05em", margin: "0 0 6px" }}>
              {tournament?.title}
            </h1>
            <div style={{ fontSize: 13, color: meta.accent, fontWeight: 700 }}>{meta.label}</div>
          </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 20 }}>
            <div style={infoBox}>
              ...
              <div style={{ fontSize: 22, fontWeight: 900, color: "#f59e0b", fontFamily: "'Bebas Neue', cursive" }}>
            ...
            <div style={infoBox}>
              <div style={{ fontSize: 20, marginBottom: 4 }}>⏱</div>
              <div style={{ fontSize: 22, fontWeight: 900, color: meta.accent, fontFamily: "'Bebas Neue', cursive" }}>
                {formatTime(secsLeft)}
              </div>
          ...
          {/* Règles */}
          <div style={{
            background: "#0a0c1080", border: "1px solid #1e2130",
            borderRadius: 12, padding: "13px 15px", marginBottom: 22,
            fontSize: 12, color: "#6b7280", lineHeight: 1.7,
          }}>
            🎟 Ticket utilisé · {gameType === "flags" ? "10 questions · Bonus combo inclus" : gameType === "memory" ? "10 rounds · Mode Normal" : "Survis le plus longtemps"}
          </div>

          {/* Warning anti-triche */}
          <div style={{
            background: "#1a1209", border: "1px solid #f59e0b25",
            borderRadius: 12, padding: "11px 14px", marginBottom: 22,
            display: "flex", alignItems: "flex-start", gap: 10,
            fontSize: 12, color: "#8b6f3a", lineHeight: 1.6,
          }}>
            <span style={{ fontSize: 16, flexShrink: 0 }}>⚠️</span>
            <span>Si tu quittes en cours de partie, ton <strong style={{ color: "#f59e0b" }}>score actuel sera soumis automatiquement</strong>. Tu ne pourras pas rejouer.</span>
          </div>
          ...
          <button
            ...
            style={{
              ...
              cursor: "pointer", boxShadow: `0 6px 24px ${meta.accent}30`,
            }}
          >
            LANCER LA PARTIE →
          </button>
      ...
      <div style={{ ...pageWrap, background: `radial-gradient(ellipse at 50% 0%, ${meta.bg} 0%, #0a0c10 60%)` }}>
        ...
        <div style={{
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            ...
            <div>
              <div style={{ fontSize: 11, color: meta.accent, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.08em" }}>
                EN COURS
              </div>
          ...
          <div style={{ textAlign: "right" }}>
            ...
            {partialScore > 0 && (
              <div style={{ fontSize: 11, color: "#4a5568" }}>
                Score actuel : <span style={{ color: meta.accent }}>{partialScore}</span>
              </div>
            )}
          </div>
        </div>
        ...
        <div style={card}>
          {gameType === "flags" && (
            <GameFlags
              ...
              tournamentEndsAt={tournament!.ends_at}
            />
          )}
          {gameType === "memory" && (
            <GameMemory
  ...
  if (pageState === "done") {
    ...
    const rankLabel = rank === 1 ? "VICTOIRE !" : rank === 2 ? "2ÈME PLACE !" : "BELLE PARTIE !";
    const rankColor = rank === 1 ? "#f59e0b" : rank === 2 ? "#9ca3af" : "#f0ede6";
        ...
        <div style={{ ...pageWrap, background: `radial-gradient(ellipse at 50% 0%, ${meta.bg} 0%, #0a0c10 60%)` }}>
          <div style={card}>
            <div style={{ textAlign: "center", marginBottom: 24 }}>
              <div style={{ fontSize: 60, marginBottom: 12, animation: "scaleIn 0.5s cubic-bezier(0.34,1.56,0.64,1)" }}>
                {rankEmoji}
              </div>
              <h2 style={{
                fontFamily: "'Bebas Neue', cursive", fontSize: 32,
                letterSpacing: "0.06em", margin: "0 0 8px", color: rankColor,
              }}>
                {rankLabel}
              </h2>
              <div style={{ fontSize: 13, color: meta.accent, fontWeight: 700 }}>{meta.label}</div>
            ...
            {rank && (
              <div style={{
                ...
                borderRadius: 12, padding: "14px 16px", textAlign: "center", marginBottom: 12,
              }}>
                ...
                {isTop2 && (
                  <div style={{ fontSize: 12, color: "#4a5568", marginTop: 4 }}>
                    Si tu gardes cette place → <span style={{ color: "#f59e0b", fontWeight: 700 }}>
                      +{rank === 1 ? tournament?.prize_1 : tournament?.prize_2} GDS
                    </span> automatiquement
                  </div>
                )}
              </div>
            )}
            ...
            <div style={{
              background: "#1a1d2a", borderRadius: 10,
              padding: "12px 14px", marginBottom: 20,
              fontSize: 12, color: "#6b7280", textAlign: "center", lineHeight: 1.6,
            }}>
              Classement final après la clôture du tournoi.<br />
...
const btnSecondary: React.CSSProperties = {
  ...
  border: "1px solid #1e2130", borderRadius: 12,
  color: "#8892a4", padding: "14px",
...
// ─────────────────────────────────────────────
// HTML TANK (inchangé, juste ajout postMessage score live)
// ─────────────────────────────────────────────
function getTankHTML(): string {
  return `<!DOCTYPE html>
<html lang="fr">
<head>
  <meta charset="UTF-8"/>
...
<div id="menu" class="menu">
  <div class="card">
    ...
    <p>🏆 Mode Tournoi — Survis aux vagues, accumule un maximum de points !</p>
...
let bossSpeech=null;
const P={x:0,y:0,r:18,ang:-Math.PI/2,spin:1.82,spd:188,lives:5,inv:0,fireCd:0,fireRate:.42,burst:3,burstRegen:0,bPower:1,multiAmmo:0,shield:0,boost:0,type:'classic',color:'#4effd1',accent:'#d9fff7',weapon:'canon',wAmmo:0,recoil:0,trackT:0};
...
applyTheme();
function resetGame(){selMap=Math.floor(Math.random()*MAPS.length);const s=SKINS[selSkin];Object.assign(P,{x:W/2,y:H/2,ang:-Math.PI/2,lives:5,inv:0,shield:0,boost:0,fireCd:0,burst:3,burstRegen:0,bPower:1,multiAmmo:0,weapon:'canon',wAmmo:0,recoil:0,trackT:0,color:s.color,accent:s.accent,type:s.type,spd:s.spd,fireRate:s.fr});rotDir=1;score=0;wave=1;bullets=[];eBullets=[];enemies=[];powerUps=[];particles=[];tracks=[];smokeArr=[];floatTxts=[];hitRings=[];bossSpeech=null;combo=0;comboTimer=0;maxCombo=0;shake=0;slowMo=0;tScale=1;makeObs();makeDecor();spawnWave();updateHUD();updateWeaponHUD();applyTheme();}
...
function damagePlayer(n){if(P.inv>0)return;if(P.shield>0){P.shield=Math.max(0,P.shield-1.5*n);sndHit();}else{P.lives=Math.max(0,P.lives-1);P.inv=1.15;combo=0;sndExplode();}shake=10;burst(P.x,P.y,P.shield>0?'#7db4ff':'#ff5c7a',24,5);updateHUD();if(P.lives<=0)gameOver();}
function killEnemy(i){const e=enemies[i],boss=e.isBoss;combo++;comboTimer=2.6;maxCombo=Math.max(maxCombo,combo);const base=boss?1000+wave*80:100+wave*15;const bonus=combo>1?Math.floor(base*Math.min(.75,combo*.08)):0;score+=base+bonus;slowMo=boss?.42:.16;sndExplode();burst(e.x,e.y,e.color,boss?70:30,boss?7:4.8);floatText(e.x,e.y-22,\`+\${base+bonus}\${combo>1?' COMBO x'+combo:''}\`,boss?'#ffcf4d':'#fff');spawnPower(e.x,e.y,boss);enemies.splice(i,1);updateHUD();
  // Notifier le score en temps réel
  notifyParent('TANK_SCORE_UPDATE',{score});
}
function updatePowers(dt){for(let i=powerUps.length-1;i>=0;i--){const p=powerUps[i];p.life-=dt;p.pulse+=dt*7;if(p.life<=0){powerUps.splice(i,1);continue;}if(Math.hypot(p.x-P.x,p.y-P.y)<p.r+P.r+4){if(p.kind==='life')P.lives=Math.min(5,P.lives+1);if(p.kind==='rapid')P.boost=7;if(p.kind==='shield')P.shield=7;if(p.kind==='multi'){P.bPower=3;P.multiAmmo=5;}if(p.kind==='missile')setWeapon('missile');sndPower();burst(p.x,p.y,p.color,30,5);powerUps.splice(i,1);updateHUD();updateWeaponHUD();}}}
...
function updateHUD(){document.getElementById('lives').textContent='♥'.repeat(Math.max(0,P.lives));document.getElementById('score').textContent=score;document.getElementById('wave').textContent=wave;if(score>bestScore){bestScore=score;localStorage.setItem('mtBest',bestScore);document.getElementById('best').textContent=bestScore;}}
