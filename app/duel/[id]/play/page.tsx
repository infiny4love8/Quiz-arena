...
type GamePhase = "countdown" | "playing" | "waiting" | "result";
...
// Seed déterministe basée sur le duelId → mêmes questions pour les deux
...
export default function DuelPlayPage() {
  ...
  useEffect(() => {
    const init = async () => {
      ...
      if (!["playing", "finished"].includes(duelData.status)) {
        router.push(`/duel/${duelId}/negotiate`);
        return;
      }
      ...
      // Mêmes questions pour les deux grâce au seed
      setQuestions(prepareQuestions(duelId));

      // Si le duel est déjà fini (je reviens sur la page)
  ...
  const finishGame = async (finalAnswers: AnswerState[]) => {
    ...
    if (oppScore !== null) {
      // Les deux ont fini → calculer le vainqueur
      const myScore = correct;
      const winnerId = myScore > oppScore
        ? myId
        : oppScore > myScore
        ? (myRole === "a" ? updated.player_b : updated.player_a)
        : null; // null = match nul
      ...
      // Gérer les coins si pari
      if (updated.bet_a && updated.bet_b && updated.bet_confirmed_a && updated.bet_confirmed_b) {
        const pot = updated.bet_a + updated.bet_b;
        const gain = Math.floor(pot * 0.9);
        if (winnerId) {
          // Déduire la mise des deux et créditer le gagnant
          await supabase.rpc("transfer_bet_coins", { winner_id: winnerId, amount: gain, loser_a: updated.player_a, loser_b: updated.player_b, bet_a: updated.bet_a, bet_b: updated.bet_b });
        }
        // Si nul : personne ne gagne, on restitue (ou on garde — à toi de choisir)
      }

      setResultData({ myScore, opponentScore: oppScore, winner: winnerId });
      setPhase("result");
    } else {
      // L'adversaire n'a pas encore fini
      setPhase("waiting");
    }
  };
        ...
        <div className="relative text-center">
          ...
          <div
            ...
            style={{
              ...
              color: countdown === 0 ? "#f97316" : "white",
            }}
          >
            {countdown === 0 ? "GO!" : countdown}
          </div>
          <p className="text-white/30 mt-6 text-sm">Même questions, chacun de son côté</p>
        </div>
        <style jsx>{`
          @keyframes countPop {
      ...
      <main className="min-h-screen bg-[#0a0a0a] text-white flex items-center justify-center">
        <div className="text-center max-w-sm px-5">
          ...
          <h2 className="text-2xl font-black mb-3">
            Tu as fini ! <span className="text-orange-400">{correctCount}/{questions.length}</span>
          </h2>
          ...
          {opponentDone && (
            <p className="mt-4 text-orange-400 text-sm animate-pulse">
              {opponentName} vient de finir ! Calcul du résultat...
            </p>
          )}
        </div>
      </main>
  ...
  if (phase === "result" && resultData) {
    ...
    const phrases = {
      win: ["Incroyable ! Tu les as dominés 🔥", "Champion ! Personne ne t'arrête 💪", "Trop fort(e), victoire écrasante ⚡"],
      lose: ["Bien joué, mais c'est pas fini... 💀", "Revanche ? Tu peux faire mieux ! 🎯", "L'adversaire était solide, next time 🤝"],
      draw: ["Égalité parfaite ! Vous êtes au même niveau 🤝", "Match nul — ni gagnant ni perdant ⚖️", "Quand deux champions se rencontrent... 🏆"],
    };
    const category = isDraw ? "draw" : isWinner ? "win" : "lose";
    const phrase = phrases[category][Math.floor(Math.random() * 3)];
    ...
    return (
      <main className="min-h-screen bg-[#0a0a0a] text-white overflow-hidden">
        <div className="fixed inset-0 pointer-events-none">
          <div
            className="absolute top-0 left-1/2 -translate-x-1/2 w-[600px] h-[400px] blur-[120px] rounded-full opacity-20"
        ...
        <div className="relative z-10 max-w-lg mx-auto px-5 py-12">
          ...
          <div className="text-center mb-10">
            ...
            <div
              ...
              style={{ filter: "drop-shadow(0 0 30px currentColor)", animation: "popIn 0.6s cubic-bezier(0.34,1.56,0.64,1)" }}
            >
              {isDraw ? "🤝" : isWinner ? "🏆" : "💀"}
            </div>
            <h1
              className="text-4xl font-black mb-2"
              style={{ color: isDraw ? "#f59e0b" : isWinner ? "#22c55e" : "#ef4444" }}
            >
            ...
            <div className="grid grid-cols-3 items-center gap-4">
              ...
              <div className="text-center">
                ...
                <p
                  ...
                  style={{ color: isDraw ? "#f59e0b" : isWinner ? "#22c55e" : "white" }}
                >
                  {myScore}
                </p>
                <p className="text-xs text-white/20 mt-1">/{questions.length}</p>
              </div>
          ...
          {hasBet && (
            ...
            >
              ...
              {isDraw ? (
                <p className="text-white/60 text-sm">Match nul — mises restituées</p>
              ) : isWinner ? (
                <>
                  <p className="text-3xl font-black text-green-400">+{gain} 🪙</p>
                  <p className="text-xs text-white/30 mt-1">90% du pot de {pot} coins</p>
                </>
              ) : (
                <>
                  <p className="text-3xl font-black text-red-400">-{myRole === "a" ? duel!.bet_a : duel!.bet_b} 🪙</p>
                  <p className="text-xs text-white/30 mt-1">Mise perdue</p>
                </>
              )}
            </div>
          )}
          ...
          <div className="rounded-2xl border border-white/6 bg-white/[0.02] p-5 mb-8">
            ...
            <div className="grid grid-cols-5 gap-2">
              {answers.map((a, i) => (
                <div
                  key={i}
                  className="aspect-square rounded-xl flex items-center justify-center text-sm font-black"
                  style={{
                    backgroundColor: a.correct ? "rgba(34,197,94,0.15)" : "rgba(239,68,68,0.15)",
      ...
      <div className="max-w-lg mx-auto">
        ...
        <div className="flex items-center justify-between mb-5">
          <div className="flex items-center gap-3">
            ...
            {combo > 1 && (
              <span className="text-xs px-2 py-1 rounded-lg bg-orange-500/15 border border-orange-500/30 text-orange-400 font-black animate-pulse">
                🔥 x{combo}
              </span>
            )}
          </div>
        ...
        {/* Score live + adversaire */}
        <div className="flex items-center justify-between mb-6 rounded-xl border border-white/6 bg-white/[0.02] px-4 py-3">
          <div className="text-center">
            <p className="text-xs text-white/25 mb-0.5">Toi</p>
            <p className="font-black text-green-400">{correctCount}</p>
          </div>
          <div className="text-white/15 font-black">VS</div>
          <div className="text-center">
            <p className="text-xs text-white/25 mb-0.5">{opponentName}</p>
            <p className="font-black text-white/40">
              {opponentDone ? "✓ fini" : "..."}
            </p>
          </div>
        </div>
        ...
        <div
          key={animKey}
          className="rounded-3xl border border-white/8 bg-white/[0.03] p-6 mb-5"
